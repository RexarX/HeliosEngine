...
namespace Helios {
  ...
  Entity& Entity::GetParent() const {
    if (m_Scene == nullptr) { CORE_ASSERT(false, "Scene is null!"); return m_Scene->m_InvalidEntity; }
    if (!m_Scene->m_Registry.valid(m_Entity)) { CORE_ASSERT(false, "Entity is not valid!"); return m_Scene->m_InvalidEntity; }
    Entity* parent = m_Scene->m_Registry.get<Relationship>(m_Entity).parent;
    if (parent == nullptr) { CORE_ASSERT(false, "Entity has no parent!"); return m_Scene->m_InvalidEntity; }
