...
namespace Helios
{
  ...
  {
    m_SortedSystems.reserve(other.m_SortedSystems.size());
    for (const auto& system : other.m_SortedSystems) {
      SystemEntry entry;
      entry.priority = system.priority;
      entry.system = system.system->Clone();
      entry.type = system.type;
      m_SortedSystems.push_back(std::move(entry));
    }
  }
  ...
  ECSManager::ECSManager(ECSManager&& other) noexcept
    ...
    m_SortedSystems(std::move(other.m_SortedSystems))
