...
namespace Helios {
	...
	void LinuxWindow::Init() {
		...
		switch (windowMode) {
			...
			case Mode::Unspecified: {
				CORE_ASSERT(false, "Window mode 'None' is not supported!");
				break;
			}
		...
		glfwSetWindowSizeCallback(m_Window, [](GLFWwindow* window, int width, int height) {
			LinuxWindow& win = *static_cast<LinuxWindow*>(glfwGetWindowUserPointer(window));
  ...
  void LinuxWindow::Shutdown() {
	...
	void LinuxWindow::ShutdownImGui() {
		m_Context->ShutdownImGui();
	}

	void LinuxWindow::BeginFrameImGui() {
		m_Context->BeginFrameImGui();
	}

	void LinuxWindow::EndFrameImGui() {
		m_Context->EndFrameImGui();
	}

	void LinuxWindow::SetState(State state) {
	...
	void LinuxWindow::SetMode(Mode mode) {
	...
	void LinuxWindow::SetResolution(uint32_t resX, uint32_t resY) {
	...
	void LinuxWindow::SetPosition(uint32_t x, uint32_t y) {
		auto& [posX, posY] = m_Properties.position;
	...
	void LinuxWindow::SetRefreshRate(uint32_t refreshRate) {
	...
	void LinuxWindow::UpdateMonitor() {
	...
	void LinuxWindow::MonitorCallback(GLFWmonitor* monitor, int event) {
		LinuxWindow& win = *static_cast<LinuxWindow*>(glfwGetMonitorUserPointer(monitor));
