...
namespace VoxelEngine
{
	...
	public:
		...
		virtual const uint32_t GetSlot() const = 0;
		...
		static std::shared_ptr<Texture> Create(const std::string& name, const TextureSpecification& specification);
		static std::shared_ptr<Texture> Create(const std::string& name, const std::string& path);

		static std::shared_ptr<Texture> CreateArray(const std::string& name, const std::vector<std::string>& paths);
