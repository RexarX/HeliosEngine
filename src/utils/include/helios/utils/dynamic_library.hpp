 ...
 * @details Provides a unified interface for loading dynamic libraries (DLLs on
 * Windows, .so on Linux, .dylib on macOS) and retrieving function symbols.
 *
 * @note Not thread-safe. External synchronization required for concurrent
 * access.
 *
 * @code
 * DynamicLibrary lib;
