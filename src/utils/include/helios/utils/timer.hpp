 ...
 * @tparam Clock Clock type used to measure time. Must satisfy
 * `ClockTrait`.
 */
template <ClockTrait Clock = std::chrono::steady_clock>
class Timer {
public:
  ...
  constexpr Timer(const Timer&) noexcept(
      std::is_nothrow_copy_constructible_v<Clock>) = default;
  constexpr Timer(Timer&&) noexcept(
      std::is_nothrow_move_constructible_v<Clock>) = default;
  constexpr ~Timer() noexcept(std::is_nothrow_destructible_v<Clock>) = default;

  constexpr Timer& operator=(const Timer&) noexcept(
      std::is_nothrow_copy_assignable_v<Clock>) = default;
  constexpr Timer& operator=(Timer&&) noexcept(
      std::is_nothrow_move_assignable_v<Clock>) = default;
   ...
   * @details This is the primary API for time measurement and should be
   * preferred when working with `std::chrono`.
   * @tparam Units Duration type to convert to. Must satisfy
   * `DurationTrait`. Defaults to `Clock::duration`.
   * @return Elapsed time since last reset as `Units`.
   */
  template <DurationTrait Units = Duration>
  [[nodiscard]] constexpr Units ElapsedDuration() const;

  /**
   * @brief Get elapsed time converted to an arithmetic value with specified
   * units.
   * @details This method allows querying elapsed time in arbitrary precision
   ...
   */
  [[nodiscard]] constexpr double ElapsedSec() const {
   ...
   */
  [[nodiscard]] constexpr int64_t ElapsedMicroSec() const {
   ...
   */
  [[nodiscard]] constexpr int64_t ElapsedNanoSec() const {
...
template <ClockTrait Clock>
template <DurationTrait Units>
...
template <ClockTrait Clock>
template <utils::ArithmeticTrait Type, DurationTrait Units>
