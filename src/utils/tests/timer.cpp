...
TEST_SUITE("helios::utils::Timer") {
  ...
  TEST_CASE("utils::Timer::ElapsedDuration: Custom types and units") {
    ...
    SUBCASE("Integer microseconds") {
      const auto elapsed_us =
          timer.Elapsed<int64_t, std::chrono::microseconds>();
      CHECK_GT(elapsed_us, 0);
    }

    SUBCASE("Floating seconds") {
      const auto elapsed_sec =
          timer.Elapsed<double, std::chrono::duration<double>>();
  ...
  TEST_CASE("utils::Timer::Elapsed: Convenience helpers") {
    ...
    // Coarse consistency checks: larger units should not be smaller than
    // smaller units when converted back approximately.
  ...
  TEST_CASE("utils::Timer::ctor: Custom clock type") {
    ...
    SUBCASE("Initial is zero") {
      const auto elapsed_ns =
          timer.Elapsed<int64_t, std::chrono::nanoseconds>();
      CHECK_EQ(elapsed_ns, 0);
    }

    SUBCASE("After advancing clock") {
      TestClock::now_value =
          TestClock::time_point(TestClock::duration(1000));  // 1000 ns
      const auto elapsed_ns =
          timer.Elapsed<int64_t, std::chrono::nanoseconds>();
    ...
    SUBCASE("Reset updates start point") {
      TestClock::now_value =
          TestClock::time_point(TestClock::duration(5000));  // 5000 ns
      timer.Reset();
      TestClock::now_value =
          TestClock::time_point(TestClock::duration(8000));  // 3000 ns later
      const auto elapsed_ns =
          timer.Elapsed<int64_t, std::chrono::nanoseconds>();
    ...
    SUBCASE("Start returns underlying time point") {
      ...
      TestClock::now_value =
          TestClock::time_point(TestClock::duration(100));  // move forward
