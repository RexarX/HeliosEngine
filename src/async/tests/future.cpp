...
TEST_SUITE("helios::async::Future") {
  TEST_CASE("async::Future: construction and basic properties") {
    ...
    SUBCASE("Move construction") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Moved future did not complete within timeout");
    }

    SUBCASE("Move assignment") {
      Executor executor(1);
      TaskGraph graph("MoveAssignTest");
      graph.EmplaceTask(
      ...
      auto status = future2.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Move-assigned future did not complete within timeout");
    }
  }
  ...
  TEST_CASE("async::Future: result retrieval") {
    ...
    SUBCASE("Get with int return type") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Int return task did not complete within timeout");
    ...
    SUBCASE("Get with string return type") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Complex object task did not complete within timeout");
  ...
  TEST_CASE("async::Future: waiting functionality") {
    ...
    SUBCASE("Wait for completion") {
      TaskGraph graph("WaitForCompletion");
      ...
      REQUIRE_MESSAGE(
          ...
          "Wait for completion task did not complete within timeout");
    ...
    SUBCASE("WaitFor with timeout") {
      std::atomic<bool> should_complete{false};
      TaskGraph graph("WaitForTimeout");
    ...
    SUBCASE("WaitUntil with absolute time") {
      std::atomic<bool> should_complete{false};
      TaskGraph graph("WaitUntilTimeout");
    ...
    SUBCASE("Immediate completion") {
      TaskGraph graph("ImmediateCompletion");
      graph.EmplaceTask([]() {});
      auto future = executor.Run(graph);
      ...
      REQUIRE_MESSAGE(
          status == std::future_status::ready,
          "Immediate completion task did not complete within timeout");
    }
  }
  ...
  TEST_CASE("async::Future::Cancel") {
    ...
    SUBCASE("Cancel before execution") {
      ...
      std::atomic<int> result{0};
      TaskGraph graph("CancelBeforeExecution");
      ...
      // If cancellation failed, the task should complete and we can check the
      // result
      if (!cancelled) {
        REQUIRE_MESSAGE(status == std::future_status::ready,
                        "Non-cancelled task did not complete within timeout");
        CHECK_EQ(result.load(), 42);
      }
    }
    ...
    SUBCASE("Cancel completed task") {
      std::atomic<int> result{0};
      TaskGraph graph("CancelCompleted");
      graph.EmplaceTask([&result]() { result = 42; });
      auto future = executor.Run(graph);
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Cancel completed task did not complete within timeout");
  ...
  TEST_CASE("async::Future::Valid") {
    ...
    SUBCASE("Valid future lifecycle") {
      TaskGraph graph("ValidLifecycle");
      graph.EmplaceTask([]() {});
      auto future = executor.Run(graph);
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Valid lifecycle task did not complete within timeout");
      CHECK(future.Valid());  // Still valid after wait
    }

    SUBCASE("Invalid future operations") {
      Future<int> invalid_future;
  ...
  TEST_CASE("async::Future: multiple futures coordination") {
    ...
    SUBCASE("Multiple independent futures") {
      ...
      for (int i = 0; i < NUM_FUTURES; ++i) {
        ...
        auto status = future.WaitFor(std::chrono::milliseconds(1000));
        REQUIRE_MESSAGE(status == std::future_status::ready,
                        std::format("Multiple independent future {} did not "
    ...
    SUBCASE("Future dependency chain") {
      ...
      TaskGraph graph("DependencyChain");
      auto task1 = graph.EmplaceTask([&value1]() { value1 = 10; });
      auto task2 = graph.EmplaceTask(
          [&value1, &value2]() { value2 = value1.load() * 2; });
      auto task3 = graph.EmplaceTask(
          [&value2, &final_result]() { final_result = value2.load() + 5; });
      ...
      REQUIRE_MESSAGE(
          status == std::future_status::ready,
          "Future dependency chain did not complete within timeout");
  ...
  TEST_CASE("async::Future: exception handling") {
    ...
    SUBCASE("Future with throwing task") {
      ...
      try {
        auto status = future.WaitFor(std::chrono::milliseconds(1000));
        REQUIRE_MESSAGE(status == std::future_status::ready,
                        "Throwing task did not complete within timeout");
      } catch (...) {
    ...
    SUBCASE("Wait on throwing task") {
      ...
      TaskGraph graph("DelayedThrow");
      graph.EmplaceTask([&task_executed]() {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        task_executed = true;
        // For TaskGraph, we focus on task completion rather than exception
        // propagation
      });
      auto future = executor.Run(graph);
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Delayed throw task did not complete within timeout");
      CHECK(future.Valid());
