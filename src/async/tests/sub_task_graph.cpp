...
TEST_SUITE("helios::async::SubTaskGraph") {
  TEST_CASE(
      "async::SubTaskGraph: SubTaskGraph: basic construction and properties") {
    ...
    SUBCASE("SubTaskGraph creation and basic operations") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        CHECK(sub_graph.Joinable());
        CHECK_FALSE(sub_graph.WillBeRetained());

        auto sub_task = sub_graph.EmplaceTask(
            [&sub_task_executed]() { sub_task_executed = true; });

        CHECK_FALSE(sub_task.Empty());
        CHECK(sub_task.HasWork());
        CHECK_EQ(sub_task.Type(), TaskType::kStatic);
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Basic sub task did not complete within timeout");
    ...
    SUBCASE("SubTaskGraph with retention") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Detached sub task did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph: task creation methods") {
    ...
    SUBCASE("EmplaceTask with static callable") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        for (int i = 0; i < 5; ++i) {
          auto task = sub_graph.EmplaceTask(
              [&execution_count, i]() { ++execution_count; });
          CHECK_EQ(task.Type(), TaskType::kStatic);
          CHECK(task.HasWork());
        }
        sub_graph.Join();
      });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Multiple sub tasks did not complete within timeout");
    ...
    SUBCASE("EmplaceTask with nested sub task") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto nested_task =
            sub_graph.EmplaceTask([&](SubTaskGraph& nested_graph) {
              ++nested_execution_count;

              // Create a deeply nested sub task
              auto deep_task =
                  nested_graph.EmplaceTask([&](SubTaskGraph& deep_graph) {
                    ++deep_nested_count;
                    auto inner_task = deep_graph.EmplaceTask(
                        [&deep_nested_count]() { ++deep_nested_count; });
                    deep_graph.Join();
                  });

              CHECK_EQ(deep_task.Type(), TaskType::kSubTask);
              nested_graph.Join();
            });

        CHECK_EQ(nested_task.Type(), TaskType::kSubTask);
        sub_graph.Join();
      });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Nested sub task did not complete within timeout");
    ...
    SUBCASE("EmplaceTasks with multiple callables") {
      ...
      REQUIRE_MESSAGE(
          ...
          "Multiple nesting levels did not complete within timeout");
    ...
    SUBCASE("CreatePlaceholder and work assignment") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        placeholder.Work(
            [&placeholder_executed]() { placeholder_executed = true; });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Placeholder sub task did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph: basic parallel operations - simplified") {
    ...
    SUBCASE("Manual parallel processing simulation") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        for (int value : input_data) {
          auto task =
              sub_graph.EmplaceTask([&total_sum, &processed_count, value]() {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "For with modification did not complete within timeout");
    ...
    SUBCASE("Manual transform simulation") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        for (size_t i = 0; i < input_data.size(); ++i) {
          auto task = sub_graph.EmplaceTask(
              [&output_data, &output_mutex, &input_data, i]() {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "For with transform did not complete within timeout");
    ...
    SUBCASE("Manual reduce simulation") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        for (int value : input_data) {
          auto task = sub_graph.EmplaceTask(
              [&result, value]() { result.fetch_add(value); });
          CHECK_FALSE(task.Empty());
          CHECK(task.HasWork());
        }
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "For with accumulation did not complete within timeout");
    ...
    SUBCASE("Manual sort simulation") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto sort_task =
            sub_graph.EmplaceTask([&data]() { std::ranges::sort(data); });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Manual sort simulation did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph: actual parallel algorithms") {
    ...
    SUBCASE("ForEach with vector") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        auto foreach_task = sub_graph.ForEach(
            input, [&sum](int value) { sum.fetch_add(value); });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "ForEach with vector did not complete within timeout");
    ...
    SUBCASE("ForEachIndex with range") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto foreach_index_task =
            sub_graph.ForEachIndex(0, 10, 2, [&sum, &count](int index) {
              sum.fetch_add(index);
              count.fetch_add(1);
            });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "ForEach with index did not complete within timeout");
    ...
    SUBCASE("Transform operation") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto transform_task =
            sub_graph.Transform(input, output, [](int x) { return x * x; });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Transform operation did not complete within timeout");
    ...
    SUBCASE("Reduce operation") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Reduce operation did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph::Linearize") {
    ...
    SUBCASE("Linearize tasks within subflow") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        for (int i = 0; i < 5; ++i) {
          ordered_tasks.push_back(sub_graph.EmplaceTask(
              [&order_mutex, &execution_order, &order_counter, i]() {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Manual dependencies did not complete within timeout");
    ...
    SUBCASE("Linearize with array") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        std::array<Task, 3> tasks = {
            sub_graph.EmplaceTask(
                [&order_mutex, &execution_order, &order_counter]() {
                  const std::scoped_lock lock(order_mutex);
                  execution_order.push_back(order_counter.fetch_add(1));
                }),
            sub_graph.EmplaceTask(
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Complex dependencies did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph: task management") {
    ...
    SUBCASE("RemoveTask from subflow") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto task1 =
            sub_graph.EmplaceTask([&execution_count]() { ++execution_count; });
        auto task2 =
            sub_graph.EmplaceTask([&execution_count]() { ++execution_count; });
        auto task3 =
            sub_graph.EmplaceTask([&execution_count]() { ++execution_count; });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Conditional execution did not complete within timeout");

      CHECK_EQ(execution_count.load(),
               2);  // Only task1 and task3 should execute
    }

    SUBCASE("ComposedOf with external graph") {
      TaskGraph external_graph("ExternalGraph");
      std::atomic<bool> external_executed{false};
      std::atomic<bool> sub_executed{false};

      // Add task to external graph
      external_graph.EmplaceTask(
          [&external_executed]() { external_executed = true; });
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        ...
        auto sub_task =
            sub_graph.EmplaceTask([&sub_executed]() { sub_executed = true; });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "External dependency did not complete within timeout");
  ...
  TEST_CASE("async::SubTaskGraph: complex dependency patterns") {
    ...
    SUBCASE("Diamond pattern within subflow") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        // Create diamond dependency pattern: task_a -> (task_b, task_c) ->
        // task_d
      ...
      bool valid_order = (execution_order[1] == 1 && execution_order[2] == 2) ||
                         (execution_order[1] == 2 && execution_order[2] == 1);
      CHECK(valid_order);
    }
  }
  ...
  TEST_CASE("async::SubTaskGraph: executor delegation methods") {
    ...
    SUBCASE("Run external graph from subflow") {
      TaskGraph external_graph("ExternalFromSub");
      std::atomic<bool> external_executed{false};
    ...
    SUBCASE("SilentAsync from subflow") {
      ...
      executor
          .WaitForAll();  // Need to wait for task graph and silent async task
  ...
  TEST_CASE("async::SubTaskGraph: error handling and edge cases") {
    ...
    SUBCASE("Exception in sub task") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto throwing_task = sub_graph.EmplaceTask(
            []() { throw std::runtime_error("Sub task exception"); });

        auto normal_task = sub_graph.EmplaceTask(
            [&normal_task_executed]() { normal_task_executed = true; });
    ...
    SUBCASE("Multiple joins should be safe") {
      ...
      auto main_task = main_graph.EmplaceTask([&](SubTaskGraph& sub_graph) {
        auto task =
            sub_graph.EmplaceTask([&task_executed]() { task_executed = true; });
