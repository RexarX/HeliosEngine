...
TEST_SUITE("helios::async::TaskGraph") {
  ...
  TEST_CASE("async::TaskGraph: task creation") {
    ...
    SUBCASE("EmplaceTask with static callable") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "EmplaceTask did not complete within timeout");
    ...
    SUBCASE("EmplaceTask with sub task") {
      ...
      auto task = graph.EmplaceTask([&executed](SubTaskGraph& sub_graph) {
        executed = true;
        sub_graph.Join();
      });
      ...
      CHECK_FALSE(task.Empty());
      CHECK(task.HasWork());
      CHECK_EQ(task.Type(), TaskType::kSubTask);
      CHECK_FALSE(graph.Empty());
      CHECK_EQ(graph.TaskCount(), 1);
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Named task did not complete within timeout");
    ...
    SUBCASE("EmplaceTasks with multiple callables") {
      ...
      auto tasks =
          graph.EmplaceTasks([&execution_count]() { ++execution_count; },
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Multiple tasks did not complete within timeout");
    ...
    SUBCASE("CreatePlaceholder") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Placeholder task did not complete within timeout");
  ...
  TEST_CASE("async::TaskGraph: basic parallel operations - simplified") {
    ...
    SUBCASE("Manual parallel processing simulation") {
      ...
      for (size_t i = 0; i < input_data.size(); ++i) {
        auto task = graph.EmplaceTask(
            [&results, &results_mutex, i, value = input_data[i]]() {
    ...
    SUBCASE("Manual transform simulation") {
      ...
      for (size_t i = 0; i < input_data.size(); ++i) {
        auto task = graph.EmplaceTask([&output_data, &input_data, i]() {
    ...
    SUBCASE("Manual reduce simulation") {
      ...
      for (int value : input_data) {
        auto task =
            graph.EmplaceTask([&result, value]() { result.fetch_add(value); });
        CHECK_FALSE(task.Empty());
        CHECK(task.HasWork());
      }
    ...
    SUBCASE("Manual sort simulation") {
      ...
      auto sort_task =
          graph.EmplaceTask([&data]() { std::sort(data.begin(), data.end()); });
  ...
  TEST_CASE("async::TaskGraph: actual parallel algorithms") {
    ...
    SUBCASE("ForEach with vector") {
      ...
      auto foreach_task =
          graph.ForEach(input, [&sum](int value) { sum.fetch_add(value); });
    ...
    SUBCASE("ForEachIndex with range") {
      ...
      auto foreach_index_task =
          graph.ForEachIndex(0, 10, 2, [&sum, &count](int index) {
            sum.fetch_add(index);
            count.fetch_add(1);
          });
    ...
    SUBCASE("Transform operation") {
      ...
      auto transform_task =
          graph.Transform(input, output, [](int x) { return x * x; });
  ...
  TEST_CASE("async::TaskGraph::Linearize") {
    ...
    SUBCASE("Linearize with array of tasks") {
      ...
      for (size_t i = 0; i < task_count; ++i) {
        tasks[i] = graph.EmplaceTask([&execution_order, &order_counter]() {
          execution_order.push_back(order_counter.fetch_add(1));
        });
      }
  ...
  TEST_CASE("async::TaskGraph: task management") {
    ...
    SUBCASE("RemoveTask") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Cycle detection did not complete within timeout");
    ...
    SUBCASE("RemoveDependency") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Self dependency did not complete within timeout");
    }
  }
  ...
  TEST_CASE("async::TaskGraph: composition") {
    ...
    SUBCASE("Compose another graph") {
      ...
      composed_graph.EmplaceTask(
          [&composed_executed]() { composed_executed = true; });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Composed subgraph did not complete within timeout");
    ...
    SUBCASE("Compose with dependencies") {
      ...
      // Add task to composed graph
      composed_graph.EmplaceTask([&execution_order, &order_counter]() {
        execution_order.push_back(order_counter.fetch_add(1));
      });

      // Add tasks to main graph with dependencies
      auto task_before =
          main_graph.EmplaceTask([&execution_order, &order_counter]() {
            execution_order.push_back(order_counter.fetch_add(1));
          });
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Subgraph dependencies did not complete within timeout");
  ...
  TEST_CASE("async::TaskGraph: visitor pattern") {
    ...
    SUBCASE("Visit all tasks") {
      ...
      std::vector<Task> visited_tasks;

      graph.ForEachTask(
          [&visited_tasks](Task& task) { visited_tasks.push_back(task); });
