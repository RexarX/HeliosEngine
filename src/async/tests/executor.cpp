...
TEST_SUITE("helios::async::Executor") {
  ...
  TEST_CASE("async::Executor::Run: task graph execution") {
    ...
    SUBCASE("Run single task graph by reference") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Task graph did not complete within timeout");
    ...
    SUBCASE("Run task graph with callback") {
      ...
      auto future = executor.Run(
          graph, [&callback_executed]() { callback_executed = true; });
      auto status = future.WaitFor(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Task graph did not complete within timeout");
    ...
    SUBCASE("Run moved task graph with callback") {
      ...
      std::atomic<bool> callback_executed{false};
      auto future = executor.Run(std::move(graph), [&callback_executed]() {
  ...
  TEST_CASE("async::Executor::RunN: multiple executions") {
    ...
    SUBCASE("RunN with task graph reference") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunN did not complete within timeout");
    ...
    SUBCASE("RunN with callback") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(2000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunN with callback did not complete within timeout");

      CHECK_EQ(execution_count.load(), run_count);
      CHECK_EQ(callback_count.load(),
               1);  // Callback is called once after all runs
    }
  ...
  TEST_CASE("async::Executor::RunUntil: predicate-based execution") {
    ...
    SUBCASE("RunUntil with simple predicate") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(2000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunUntil did not complete within timeout");
    ...
    SUBCASE("RunUntil with moved graph") {
      ...
      auto future = executor.RunUntil(std::move(graph), [&execution_count]() {
        return execution_count.load() >= 3;
      });
      auto status = future.WaitFor(std::chrono::milliseconds(2000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunUntil did not complete within timeout");
    ...
    SUBCASE("RunUntil with callback") {
      ...
      auto status = future.WaitFor(std::chrono::milliseconds(2000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunUntil with callback did not complete within timeout");
    ...
    SUBCASE("RunUntil with moved graph and callback") {
      ...
      auto future = executor.RunUntil(
          std::move(graph),
          [&execution_count]() { return execution_count.load() >= 3; },
          [&callback_executed]() { callback_executed = true; });
      auto status = future.WaitFor(std::chrono::milliseconds(2000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "RunUntil with callback did not complete within timeout");
  ...
  TEST_CASE("async::Executor::Async: asynchronous task execution") {
    ...
    SUBCASE("Async with void return") {
      ...
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "Async task did not complete within timeout");
      CHECK(executed.load());
    }
    ...
    SUBCASE("Named async task") {
      ...
      auto future =
          executor.Async("NamedTask", []() { return expected_result; });
    ...
    SUBCASE("Named SilentAsync execution") {
      std::atomic<bool> executed{false};
      executor.SilentAsync("SilentNamedTask",
  ...
  TEST_CASE("async::Executor::DependentAsync: dependent async tasks") {
    ...
    SUBCASE("DependentAsync with single dependency") {
      ...
      std::vector<AsyncTask> deps = {first_task};
      ...
      REQUIRE_MESSAGE(status1 == std::future_status::ready,
                      "First dependent task did not complete within timeout");
      auto status2 = dependent_future.wait_for(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status2 == std::future_status::ready,
                      "Second dependent task did not complete within timeout");
    ...
    SUBCASE("SilentDependentAsync") {
      ...
      // Create first task
      auto first_future =
          executor.Async([&first_executed]() { first_executed = true; });

      // Create dependent task
      std::vector<AsyncTask> deps;  // Empty for this test
      auto dependent_task = executor.SilentDependentAsync(
          [&second_executed]() { second_executed = true; }, deps);

      CHECK_FALSE(dependent_task.Empty());

      auto status = first_future.wait_for(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "First task did not complete within timeout");
      executor.WaitForAll();
  ...
  TEST_CASE("async::Executor::IsWorkerThread: worker thread identification") {
    ...
    SUBCASE("Worker thread identification from task") {
      ...
      auto future = executor.Async(
          [&executor, &is_worker_from_task, &worker_id_from_task]() {
      ...
      CHECK_LT(worker_id_from_task.load(),
               static_cast<int>(executor.WorkerCount()));
    }
  }
  ...
  TEST_CASE("async::Executor::CoRun: cooperative execution") {
    ...
    SUBCASE("CoRun from worker thread") {
      ...
      auto future = executor.Async([&executor, &corun_executed]() {
        TaskGraph inner_graph("InnerGraph");
      ...
      auto status = future.wait_for(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "CoRun task did not complete within timeout");
    ...
    SUBCASE("CoRunUntil from worker thread") {
      ...
      auto status = future.wait_for(std::chrono::milliseconds(1000));
      REQUIRE_MESSAGE(status == std::future_status::ready,
                      "CoRunUntil task did not complete within timeout");
  ...
  TEST_CASE("async::Executor::IdleWorkerCount: idle and queue statistics") {
    ...
    SUBCASE("Idle worker count changes with work") {
      ...
      for (auto& future : futures) {
        ...
        REQUIRE_MESSAGE(
            status == std::future_status::ready,
            "Idle worker count task did not complete within timeout");
      }
    }
    ...
    SUBCASE("Queue count is consistent") {
      ...
      // Note: Queue count can be greater than worker count in TaskFlow's
