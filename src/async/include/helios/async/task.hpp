 ...
 * @details Task objects are lightweight handles - copying them is safe and
 * efficient. Wraps `tf::Task` and provides methods to manage task dependencies,
 * work assignment, and metadata. Tasks can be static (simple callable) or
 * dynamic (subtasks). The underlying task data is managed by the` TaskGraph`.
...
private:
  ...
  [[nodiscard]] const tf::Task& UnderlyingTask() const noexcept {
    return task_;
  }
...
inline Task& Task::Work(C&& callable) {
  HELIOS_ASSERT(!Empty(), "Cannot assign work to empty task!");
...
inline Task& Task::Precede(const R& tasks) {
  HELIOS_ASSERT(!Empty(), "Task cannot be empty!");
  auto view = tasks | std::ranges::views::filter(
                          [](const auto& task) { return !task.Empty(); });
...
inline Task& Task::Name(const std::string& name) {
  HELIOS_ASSERT(!Empty(), "Cannot assign name to empty task!");
