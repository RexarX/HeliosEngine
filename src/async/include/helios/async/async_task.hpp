 ...
 * @details Wraps a `tf::AsyncTask`, providing methods to query task state and
 * manage its lifecycle. Instances are typically returned by `Executor`'s
...
public:
  ...
  bool operator==(const AsyncTask& other) const noexcept {
    return task_.hash_value() == other.task_.hash_value();
  }
   ...
   */
  [[nodiscard]] static constexpr TaskType GetTaskType() noexcept {
...
private:
  ...
  [[nodiscard]] const tf::AsyncTask& UnderlyingTask() const noexcept {
    return task_;
  }
