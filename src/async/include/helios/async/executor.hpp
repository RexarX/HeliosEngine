...
/**
 * @brief Manages worker threads and executes task graphs using work-stealing
 * scheduling.
 * @details The Executor wraps `tf::Executor` and provides methods to run
 * `TaskGraph`s and create asynchronous tasks. It manages a pool of worker
 * threads that efficiently execute tasks using a work-stealing algorithm.
 * @note All member functions are thread-safe unless otherwise noted.
 */
class Executor {
public:
  /**
   * @brief Default constructs an executor with hardware concurrency worker
   * threads.
   */
  Executor() = default;
   ...
   */
  explicit Executor(size_t worker_thread_count)
   ...
   * @details The executor does not own the graph - ensure it remains alive
   * during execution.
   * @note Thread-safe.
  ...
  auto Run(TaskGraph& graph) -> Future<void> {
    return Future<void>(executor_.run(graph.UnderlyingTaskflow()));
  }
  ...
  auto Run(TaskGraph&& graph) -> Future<void> {
    return Future<void>(
        executor_.run(std::move(std::move(graph).UnderlyingTaskflow())));
  }
  ...
  auto Run(TaskGraph& graph, C&& callable) -> Future<void> {
    return Future<void>(
        executor_.run(graph.UnderlyingTaskflow(), std::forward<C>(callable)));
  }
   ...
   */
  auto RunN(TaskGraph&& graph, size_t count) -> Future<void> {
    return Future<void>(executor_.run_n(
  ...
  auto RunN(TaskGraph&& graph, size_t count, C&& callable) -> Future<void> {
    return Future<void>(
        executor_.run_n(std::move(std::move(graph).UnderlyingTaskflow()), count,
                        std::forward<C>(callable)));
  }
  ...
  auto RunUntil(TaskGraph& graph, Predicate&& predicate) -> Future<void> {
    return Future<void>(executor_.run_until(
  ...
  auto RunUntil(TaskGraph&& graph, Predicate&& predicate) -> Future<void> {
    return Future<void>(
        executor_.run_until(std::move(std::move(graph).UnderlyingTaskflow()),
  ...
  auto RunUntil(TaskGraph& graph, Predicate&& predicate, C&& callable)
      -> Future<void> {
    return Future<void>(executor_.run_until(graph.UnderlyingTaskflow(),
                                            std::forward<Predicate>(predicate),
                                            std::forward<C>(callable)));
  }
  ...
  [[nodiscard]] int CurrentWorkerId() const {
    return executor_.this_worker_id();
  }
  ...
  [[nodiscard]] size_t RunningTopologyCount() const {
    return executor_.num_topologies();
  }
...
template <std::invocable C>
inline auto Executor::Async(std::string name, C&& callable)
    -> std::future<std::invoke_result_t<C>> {
...
inline auto Executor::DependentAsync(C&& callable,
  ...
  auto [task, future] = executor_.dependent_async(
...
inline AsyncTask Executor::SilentDependentAsync(
  ...
  return AsyncTask(executor_.silent_dependent_async(
      std::forward<C>(callable), tf_deps.begin(), tf_deps.end()));
}

inline void Executor::WaitForAll() {
