...
/**
 * @brief Represents a task dependency graph that can be executed by an
 * Executor.
 * @details Wraps `tf::Taskflow` and provides methods to create tasks, manage
 * dependencies, and build complex computational graphs. Tasks within the graph
 * can have dependencies that determine execution order.
...
public:
  ...
  Task ForEachIndex(I start, I end, I step, C&& callable) {
    return Task(
        taskflow_.for_each_index(start, end, step, std::forward<C>(callable)));
  }
  ...
  Task Transform(const InputRange& input_range, OutputRange& output_range,
    ...
    return Task(taskflow_.transform(
...
private:
  [[nodiscard]] tf::Taskflow& UnderlyingTaskflow() noexcept {
    return taskflow_;
  }

  [[nodiscard]] const tf::Taskflow& UnderlyingTaskflow() const noexcept {
    return taskflow_;
  }
...
inline void TaskGraph::ForEachTask(const Visitor& visitor) const {
  HELIOS_ASYNC_PROFILE_SCOPE_N("helios::async::TaskGraph::ForEachTask");
...
inline Task TaskGraph::Sort(R& range, Compare&& comparator) {
  if constexpr (std::same_as<std::remove_cvref_t<Compare>, std::less<>>) {
    return Task(
        taskflow_.sort(std::ranges::begin(range), std::ranges::end(range)));
  } else {
    return Task(taskflow_.sort(std::ranges::begin(range),
                               std::ranges::end(range),
...
inline void TaskGraph::Name(std::string_view name) {
  HELIOS_ASSERT(!name.empty(), "name cannot be empty!");
