 ...
 * @details Wraps `tf::Subflow` and provides methods to create tasks dynamically
 * at runtime. `SubTaskGraph`s are spawned from the execution of a `SubTask` and
 * allow for runtime-dependent task creation and dependency management.
...
public:
   ...
   * @details Called automatically when the `SubTaskGraph` goes out of scope,
   * unless `Join()` has already been called.
   * @warning Must be called by the same worker thread that created this
   * subflow.
   */
  void Join();
  /**
   ...
   * Retaining it allows it to remain valid after being joined.
   * @warning Must be called by the same worker thread that created this
   * subflow.
  ...
  Task ForEachIndex(I start, I end, I step, C&& callable) {
    return Task(
        subflow_.for_each_index(start, end, step, std::forward<C>(callable)));
  }

  /**
   * @brief Creates a parallel transform task that applies a function to each
   * element.
   * @note Not thread-safe
  ...
  Task Transform(const InputRange& input_range, OutputRange& output_range,
    ...
    return Task(subflow_.transform(
        std::ranges::begin(input_range), std::ranges::end(input_range),
        std::ranges::begin(output_range),
        std::forward<TransformFunc>(transform_func)));
  }

  /**
   * @brief Creates a parallel reduction task that combines elements using a
   * binary operation.
   * @note Not thread-safe
   * @tparam R Range type
  ...
  template <std::ranges::random_access_range R, typename Compare = std::less<>>
    requires std::predicate<Compare, std::ranges::range_reference_t<R>,
   ...
   * @details A subflow is joinable if it has not yet been joined with its
   * parent task.
   ...
   */
  [[nodiscard]] bool WillBeRetained() const noexcept {
    return subflow_.retain();
  }
   ...
   * @details Task graph is not owned - ensure it remains alive during
   * execution.
   * @note Thread-safe.
   ...
   */
  auto Run(TaskGraph&& graph) -> Future<void> {
    return Future<void>(subflow_.executor().run(
        std::move(std::move(graph).UnderlyingTaskflow())));
  }
  ...
  auto RunN(TaskGraph& graph, size_t count) -> Future<void> {
    return Future<void>(
        subflow_.executor().run_n(graph.UnderlyingTaskflow(), count));
  }
   ...
   */
  auto RunN(TaskGraph&& graph, size_t count) -> Future<void> {
    return Future<void>(subflow_.executor().run_n(
        std::move(std::move(graph).UnderlyingTaskflow()), count));
  }

  /**
   * @brief Runs a task graph for the specified number of times and invokes a
   * callback.
   * @details Task graph is not owned - ensure it remains alive during
   * execution.
   * @note Thread-safe.
  ...
  auto RunN(TaskGraph& graph, size_t count, C&& callable) -> Future<void> {
    return Future<void>(subflow_.executor().run_n(
        graph.UnderlyingTaskflow(), count, std::forward<C>(callable)));
  }

  /**
   * @brief Runs a moved task graph for the specified number of times and
   * invokes a callback.
   * @note Thread-safe.
   * @tparam C Callable type
   * @param graph Task graph to execute (moved)
  ...
  auto RunUntil(TaskGraph& graph, Predicate&& predicate) -> Future<void> {
    return Future<void>(subflow_.executor().run_until(
        graph.UnderlyingTaskflow(), std::forward<Predicate>(predicate)));
  }
  ...
  /**
   * @brief Runs a task graph repeatedly until the predicate returns true, then
   * invokes a callback.
   * @details Task graph is not owned - ensure it remains alive during
   * execution.
   * @note Thread-safe.
   ...
   * @param predicate Boolean predicate to determine when to stop
   * @param callable Callback to invoke after execution completes
   * @return Future that completes when predicate returns true and callback
   * finishes
   */
  template <std::predicate Predicate, std::invocable C>
  auto RunUntil(TaskGraph& graph, Predicate&& predicate, C&& callable)
      -> Future<void> {
    return Future<void>(subflow_.executor().run_until(
        graph.UnderlyingTaskflow(), std::forward<Predicate>(predicate),
        std::forward<C>(callable)));
   ...
   */
  template <std::invocable C>
  auto Async(std::string name, C&& callable)
      -> std::future<std::invoke_result_t<C>> {
    return subflow_.executor().async(std::move(name),
                                     std::forward<C>(callable));
  }
  ...
  void SilentAsync(std::string name, C&& callable) {
    subflow_.executor().silent_async(std::move(name),
                                     std::forward<C>(callable));
  }

  /**
   * @brief Creates an asynchronous task that runs after specified dependencies
   * complete.
  ...
  /**
   * @brief Creates an asynchronous task that runs after dependencies complete,
  ...
  /**
   * @brief Keeps the current worker thread running until the predicate returns
   * true.
   * @warning Must be called from within a worker thread of this executor.
   * Triggers assertion if called from a non-worker thread.
   * @tparam Predicate Predicate type
   ...
   */
  [[nodiscard]] int CurrentWorkerId() const {
    return subflow_.executor().this_worker_id();
  }
  ...
  [[nodiscard]] size_t WorkerCount() const noexcept {
    return subflow_.executor().num_workers();
  }
   ...
   */
  [[nodiscard]] size_t IdleWorkerCount() const noexcept {
    return subflow_.executor().num_waiters();
  }
  ...
  [[nodiscard]] size_t QueueCount() const noexcept {
    return subflow_.executor().num_queues();
  }
   ...
   */
  [[nodiscard]] size_t RunningTopologyCount() const {
    return subflow_.executor().num_topologies();
  }
...
private:
  ...
  [[nodiscard]] const tf::Subflow& UnderlyingSubflow() const noexcept {
    return subflow_;
  }
...
inline void SubTaskGraph::Join() {
  HELIOS_ASYNC_PROFILE_SCOPE_N("helios::async::SubTaskGraph::Join");
  subflow_.join();
}

template <SubTask C>
inline Task SubTaskGraph::EmplaceTask(C&& callable) {
  return Task(subflow_.emplace(
      [callable = std::forward<C>(callable)](tf::Subflow& sf) mutable {
...
inline Task SubTaskGraph::Sort(R& range, Compare&& comparator) {
  if constexpr (std::same_as<std::remove_cvref_t<Compare>, std::less<>>) {
    return Task(
        subflow_.sort(std::ranges::begin(range), std::ranges::end(range)));
  } else {
    return Task(subflow_.sort(std::ranges::begin(range),
                              std::ranges::end(range),
...
inline auto SubTaskGraph::DependentAsync(C&& callable,
  ...
  auto [task, future] = subflow_.executor().dependent_async(
      std::forward<C>(callable), tf_deps.begin(), tf_deps.end());
...
inline AsyncTask SubTaskGraph::SilentDependentAsync(
  ...
  return AsyncTask(subflow_.executor().silent_dependent_async(
      std::forward<C>(callable), tf_deps.begin(), tf_deps.end()));
}

template <SubTask C>
inline Task TaskGraph::EmplaceTask(C&& callable) {
  return Task(taskflow_.emplace(
      [callable = std::forward<C>(callable)](tf::Subflow& subflow) mutable {
...
inline void SubTaskGraph::CoRun(TaskGraph& graph) {
  ...
  HELIOS_ASYNC_PROFILE_SCOPE_N("helios::async::SubTaskGraph::CoRun");
