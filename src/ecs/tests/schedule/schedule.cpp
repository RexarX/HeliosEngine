...
TEST_SUITE("helios::ecs::SystemSetHandle") {
  ...
  TEST_CASE("ecs::SystemSetHandle::Before") {
    ...
    SUBCASE("Before with SystemSetHandle marks schedule dirty") {
      ...
      auto second = schedule.Set(SetOne{});
...
TEST_SUITE("ecs::Schedule") {
  ...
  TEST_CASE("ecs::Schedule::Add multiple systems") {
    SUBCASE("Adding multiple systems returns an anonymous set handle") {
      Schedule schedule;

      const auto set_handle = schedule.Add(IncrementSystem{}, SetFlagSystem{});

      CHECK(schedule.IsDirty());

      const auto result = schedule.Build();
      CHECK(result.has_value());
      CHECK_NE(set_handle.Id().id, 0);
    }

    SUBCASE("Anonymous set handle supports set configuration") {
      Schedule schedule;

      auto set_handle = schedule.Add(IncrementSystem{}, SetFlagSystem{});
      set_handle.Sequence();
