...
TEST_SUITE("helios::glfw::QueryNativeHandle") {
  TEST_CASE("helios::glfw::QueryNativeHandle") {
    SUBCASE("Returns a populated native handle for a GLFW window") {
      ...
      const helios::window::NativeHandle handle =
          helios::glfw::QueryNativeHandle(*glfw_window);

#if defined(HELIOS_PLATFORM_WINDOWS)
      REQUIRE(std::holds_alternative<helios::window::Win32Handle>(handle));
      const auto& win32 = std::get<helios::window::Win32Handle>(handle);
      CHECK_NE(win32.hwnd, nullptr);
      CHECK_NE(win32.hinstance, nullptr);
#elif defined(HELIOS_PLATFORM_MACOS)
      REQUIRE(std::holds_alternative<helios::window::CocoaHandle>(handle));
      CHECK_NE(std::get<helios::window::CocoaHandle>(handle).ns_window,
               nullptr);
#elif defined(HELIOS_PLATFORM_LINUX)
      if (glfwGetPlatform() == GLFW_PLATFORM_WAYLAND) {
        REQUIRE(std::holds_alternative<helios::window::WaylandHandle>(handle));
        const auto& wayland = std::get<helios::window::WaylandHandle>(handle);
        CHECK_NE(wayland.display, nullptr);
        CHECK_NE(wayland.surface, nullptr);
      } else {
        REQUIRE(std::holds_alternative<helios::window::XlibHandle>(handle));
        const auto& x11 = std::get<helios::window::XlibHandle>(handle);
