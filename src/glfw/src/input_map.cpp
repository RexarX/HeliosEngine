...
input::Key KeyFromGlfw(int glfw_key) noexcept {
  switch (glfw_key) {
    case GLFW_KEY_SPACE:
...
int GlfwFromKey(input::Key key) noexcept {
  switch (key) {
...
auto MouseButtonFromGlfw(int button) noexcept
...
input::Modifiers ModifiersFromGlfw(int mods) noexcept {
...
input::ButtonState ButtonStateFromGlfw(int action) noexcept {
  switch (action) {
    case GLFW_PRESS:
...
input::GamepadButton GamepadButtonFromGlfw(int button) noexcept {
  if (button < 0 || button >= static_cast<int>(input::GamepadButton::kCount)) {
    return input::GamepadButton::kA;
  }
  return static_cast<input::GamepadButton>(button);
}

input::GamepadAxis GamepadAxisFromGlfw(int axis) noexcept {
...
int GlfwFromCursorIcon(input::CursorIcon icon) noexcept {
  switch (icon) {
