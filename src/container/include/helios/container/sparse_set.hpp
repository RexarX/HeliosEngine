...
/**
 * @brief A sparse set data structure for efficient mapping of sparse indices to
 * dense storage of values.
 * @details SparseSet provides O(1) insertion, deletion, and lookup operations
 * using two arrays:
 * - sparse: maps element indices to dense indices
 * - dense: stores packed values of type T in contiguous memory
 *
 * The data structure is particularly useful for managing sparse collections
 * where indices may have large gaps but you want cache-friendly iteration over
 * existing values.
 *
...
public:
   ...
   * The sparse array is reset to invalid indices but its capacity is preserved
   * for performance. Time complexity: O(sparse_capacity). Exception safety:
   * No-throw guarantee.
   */
   ...
   * @details Adds the specified value to the set at the given index if it's not
   * already present. If the index already exists, the existing value is
   ...
   * Time complexity: O(1) amortized (O(index) worst case if sparse array needs
   * resizing). Exception safety: Strong guarantee.
   * @warning Triggers assertion if index is invalid or negative.
   * @param index The index to insert at
   ...
   * @details Removes the specified index from the set using swap-and-pop
   * technique to maintain dense packing. The last element in the dense array is
   * moved to fill the gap left by the removed element. Time complexity: O(1).
   * Exception safety: No-throw guarantee.
   * @warning Triggers assertion if index is invalid, negative, or doesn't
   * exist.
   ...
   * @details Ensures that the dense array can hold at least n elements without
   * triggering a reallocation. Does not affect the sparse array capacity. Time
   * complexity: O(1) if no reallocation, O(Size()) if reallocation occurs.
   * Exception safety: Strong guarantee.
   ...
   * @details Ensures that the sparse array can accommodate indices up to
   * max_index without triggering a reallocation. Time complexity: O(1) if no
   * reallocation, O(max_index) if reallocation occurs. Exception safety: Strong
   * guarantee.
   ...
   * @details Returns a reference to the value stored at the given dense
   * position. Time complexity: O(1). Exception safety: No-throw guarantee.
   * @warning Triggers assertion if dense_index is invalid, negative, or out of
   * bounds.
   * @param dense_index The dense position to access
   * @return Reference to the value at the specified dense position
   */
  [[nodiscard]] constexpr T& GetByDenseIndex(
      DenseIndexType dense_index) noexcept;

  /**
   * @brief Gets the value at the specified dense index (const version).
   * @details Returns a const reference to the value stored at the given dense
   * position. Time complexity: O(1). Exception safety: No-throw guarantee.
   * @warning Triggers assertion if dense_index is invalid, negative, or out of
   * bounds.
   * @param dense_index The dense position to access
   ...
   */
  [[nodiscard]] constexpr const T& GetByDenseIndex(
      DenseIndexType dense_index) const noexcept;

  /**
   * @brief Tries to get the value at the specified index.
   * @details Returns a pointer to the value stored at the given index, or
   * nullptr if the index doesn't exist. Time complexity: O(1). Exception
   * safety: No-throw guarantee.
   * @param index The index to access
   * @return Pointer to the value at the specified index, or nullptr if index
   * doesn't exist
   */
  ...
  /**
   * @brief Non-member swap function for `SparseSet`.
   * @details Provides a non-member swap function to enable ADL and
   * compatibility with standard algorithms. Time complexity: O(1). Exception
   * safety: No-throw guarantee.
   ...
   * @details Two sparse sets are equal if they contain the same index-value
   * pairs. Time complexity: O(`Size()` + `other.Size()`). Exception safety:
   * No-throw guarantee.
   ...
   * @return True if the index is valid, false if it's the reserved invalid
   * value
   */
  [[nodiscard]] static constexpr bool IsValidIndex(IndexType index) noexcept {
    return index != kInvalidIndex;
  }

  /**
   * @brief Checks if an index exists in the set.
   * @details Performs a comprehensive check to ensure the index is valid and
   * exists in the set. Time complexity: O(1). Exception safety: No-throw
   * guarantee.
   * @warning Triggers assertion if index is invalid or negative.
   ...
   * @details Returns the position of the value in the dense array for the
   * specified index. Time complexity: O(1). Exception safety: No-throw
   * guarantee.
   * @warning Triggers assertion if index is invalid, negative, or doesn't
   * exist.
   ...
   */
  [[nodiscard]] constexpr DenseIndexType GetDenseIndex(
   ...
   * @details Returns the theoretical maximum number of elements the set can
   * hold. Time complexity: O(1). Exception safety: No-throw guarantee.
   ...
   * @details Returns the number of elements that can be stored in the dense
   * array without triggering a reallocation. Time complexity: O(1). Exception
   * safety: No-throw guarantee.
   ...
   * @details Returns the maximum index that can be stored without triggering a
   * sparse array reallocation. Time complexity: O(1). Exception safety:
   * No-throw guarantee.
   * @return The capacity of the sparse array
   */
  [[nodiscard]] constexpr size_type SparseCapacity() const noexcept {
...
constexpr auto SparseSet<T, IndexType, Allocator>::Insert(IndexType index,
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(index >= 0,
                  "Failed to insert value: index cannot be negative!");
  }
...
constexpr auto SparseSet<T, IndexType, Allocator>::Emplace(IndexType index,
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(index >= 0,
                  "Failed to emplace value: index cannot be negative!");
  }
...
constexpr void SparseSet<T, IndexType, Allocator>::Remove(
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(index >= 0,
                  "Failed to remove value: index cannot be negative!");
  }
  HELIOS_ASSERT(Contains(index),
                "Failed to remove value: index does not exist!");
...
constexpr void SparseSet<T, IndexType, Allocator>::ReserveSparse(
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(max_index >= 0,
                  "Failed to reserve sparse: max_index cannot be negative!");
  }
...
constexpr T& SparseSet<T, IndexType, Allocator>::GetByDenseIndex(
  ...
  if constexpr (std::is_signed_v<DenseIndexType>) {
    HELIOS_ASSERT(dense_index >= 0,
                  "Failed to get value: dense_index cannot be negative!");
  }
  HELIOS_ASSERT(dense_index < dense_.size(),
                "Failed to get value: dense_index is out of bounds!");
...
constexpr T* SparseSet<T, IndexType, Allocator>::TryGet(
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(index >= 0,
                  "Failed to try get value: index cannot be negative!");
  }
...
constexpr bool SparseSet<T, IndexType, Allocator>::Contains(
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(
        index >= 0,
        "Failed to check if set contains index: index cannot be negative!");
  }
  return static_cast<size_type>(index) < sparse_.size() &&
         sparse_[index] != kInvalidDenseIndex &&
         static_cast<size_type>(sparse_[index]) < dense_.size() &&
         static_cast<size_type>(sparse_[index]) < reverse_map_.size() &&
...
constexpr auto SparseSet<T, IndexType, Allocator>::GetDenseIndex(
  ...
  if constexpr (std::is_signed_v<IndexType>) {
    HELIOS_ASSERT(index >= 0,
                  "Failed to get dense index: index cannot be negative!");
  }
  HELIOS_ASSERT(Contains(index),
