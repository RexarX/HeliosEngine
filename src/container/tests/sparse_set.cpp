...
TEST_SUITE("helios::container::SparseSet") {
  ...
  TEST_CASE("container::SparseSet::ctor: move semantics") {
    ...
    // Move construct
    SparseSet<int> second_set(std::move(first_set));
  ...
  TEST_CASE(
      "container::SparseSet::GetDenseIndex: returns correct dense index") {
    SparseSet<int> set;
    ...
    set.Remove(
        5);  // This should move the last element (1000 -> 10000) to position 1
  ...
  TEST_CASE("container::SparseSet::Reserve: reserves capacity") {
    ...
    CHECK_GE(set.SparseCapacity(),
             1001);  // +1 because we need space for index 1000
  ...
  TEST_CASE("container::SparseSet::stress test with random operations") {
    ...
    std::mt19937 gen(42);
    std::uniform_int_distribution<size_t> dis(0, 10000);
  ...
  TEST_CASE("container::PmrSparseSet: works with memory_resource") {
    ...
    CHECK_EQ(set.Size(), 2);
    CHECK_EQ(set.Get(1), 100);
    CHECK_EQ(set.Get(2), 200);
  }
}  // TEST_SUITE
